...
%type <node> assignment_expression unary_expression postfix_expression cast_expression type_name enum_specifier struct_declarator labeled_internal
%type <node> logical_and_expression inclusive_or_expression abstract_declarator primary_expression argument_expression_list pointer labeled_statement_name
...
%type <node> statement labeled_statement expression_statement selection_statement iteration_statement jump_statement statement_list labeled_internal_list
...
declaration_specifiers
  ...
  | type_specifier_node declaration_specifiers          { $$ = new DeclarationSpecifier($1, $2); }
  | type_qualifier                                      { $$ = $1; }
...
struct_declarator
	: declarator                                     { $$ = $1; }
	| ':' constant_expression                         { $$ = new ConstantExpression(nullptr, $2); }
  | declarator ':' constant_expression               { $$ = new ConstantExpression($1, $3);}
	;

enum_specifier
  : ENUM '{' enumerator_list '}'                    { $$ = new EnumSpecifier(nullptr,$3); }
  | ENUM C_IDENTIFIER '{' enumerator_list '}'       { $$ = new EnumSpecifier($2,$4); }
  ;

enumerator_list
  : enumerator                                    { $$ = new EnumDeclarationListNode(nullptr, $1); }
  | enumerator_list ',' enumerator                { $$ = new EnumDeclarationListNode($1, $3); }

enumerator
  : C_IDENTIFIER                              { $$ = new EnumDeclaration($1, nullptr);}
  | C_IDENTIFIER '=' constant_expression      { $$ = new EnumDeclaration($1, $3);}
...
postfix_expression
	...
	| postfix_expression '[' expression ']'                        { $$ = new PostfixExpression($1, "array",$3); }
...
labeled_internal_list
  : labeled_internal        { $$ = new LabeledInternalList(nullptr, $1); }
  | labeled_internal_list labeled_internal      { $$ = new LabeledInternalList($1, $2); }

labeled_statement
	: labeled_statement_name ':' labeled_internal_list                { $$ = new LabeledStatement($1, nullptr, $3); }
	| labeled_statement_name constant_expression ':' labeled_internal_list     { $$ = new LabeledStatement($1, $2, $4); }
	| labeled_statement_name ':' labeled_internal_list                     { $$ = new LabeledStatement($1, nullptr, $3); }
	;

labeled_statement_name
  : C_IDENTIFIER                { $$ = new identifierC($1); }
  | DEFAULT                     { $$ = new DefaultNode($1); }
  | CASE                        { $$ = new CaseNode($1); }
  ;
